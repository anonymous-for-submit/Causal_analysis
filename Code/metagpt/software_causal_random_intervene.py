...
if __name__ == '__main__':
    ...
    for idx,task in enumerate(loaded_dataset):
        ...
        with open(log_path,'w+') as log_f:
            ...
            print('-'*100)
            print('generating cause, randomly select features to intervene')
            len_cause=1
            ...
            import random
            # def get_random_combination(input_list):
            #     count = random.randint(0, len(input_list))
            #     return random.sample(input_list, count)
            def get_random_combination(input_list):
                return [element for element in input_list if random.choice([True, False])]
            for total_runs in range(args.max_run):
                random_feature_queue.append(get_random_combination(feature_nodes))


            for idx,features in enumerate(random_feature_queue):
