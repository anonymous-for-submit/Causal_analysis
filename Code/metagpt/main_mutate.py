...
def mutate_one_nl(seed,args,mutate_method='random',model='gpt-4o-ca'):
    ...
    if 'human' in args.dataset:
        ...
        intent = seed.solution['docstring']
    else:
        intent = seed.solution['prompt']
    ...
    if 'human' in args.dataset:
        ...
        new_solution['prompt'] =  seed.solution['signature']+ '\n\t\"\"\"\n\t' + mutated_nl +'\n\t\"\"\"\n\t'
        new_solution['docstring']= mutated_nl
    else:
        new_solution['prompt'] = mutated_nl
...
def monitor(plan,requirement,code='',model = 'gpt-3.5-turbo-ca',task = 'repair_plan'):
    if task == 'repair_plan':
