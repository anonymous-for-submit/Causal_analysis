...
if __name__ == '__main__':
    ...
    blame_map_new = defaultdict(float)
    
    args.max_run -= len(feature_nodes)
    ...
    for idx,task in enumerate(loaded_dataset):
        ...
        with open(log_path,'w+') as log_f:
            ...
            print('-'*100)
            print('generating cause, randomly select features to intervene by length ...')
            len_cause=1
            finished_features = set()
            causes=[]
    
            generate_ids = []
            random_feature_queue = []
            import random
            ...
            for len_cause in range(2,args.max_len_cause+1):
                # print(f'valid_feature_queue len: {len(valid_feature_queue)}')
                valid_feature_queue += list(combinations(valid_feature_nodes, len_cause))
                if len(valid_feature_queue)>=args.max_run:
                    break
            valid_feature_queue = [list(i) for i in valid_feature_queue]

            # valid_feature_queue = valid_feature_queue[:args.max_run]
            random.shuffle(valid_feature_queue)
            print(f'total {len(valid_feature_queue)} feature combinations to intervene')

            for idx,features in enumerate(valid_feature_queue):
                ...
                if not res:
                    ...
                    for feature_node in features:
                        ...
                        blame_map_new[feature_node] += 1 / (len(features) **2)
                    # fails 记录连续失败的次数，找到一次False则将连续失败次数置为9=0
                    fails=0

                log_js = {
                        'total_runs':idx,
                        'len_cause':len(features),
        ...
        blame_map_path = log_dir + f'new_blame_map_from_{args.begin_idx}_to_{idx}.jsonl'
        with open(blame_map_path, 'w+') as f:
            for feature, blame in blame_map_new.items():
                f.write(json.dumps({feature.build_str(): blame}) + '\n')
                f.flush()
    ...
    print('new blame map:')
    output_path = args.output_path + 'new_blame.jsonl'
    with open(output_path, 'w+') as f:
        for feature, blame in blame_map_new.items():
            print(f'  {feature.build_str()}: {blame}')
            f.write(json.dumps({feature.build_str(): blame}) + '\n')
